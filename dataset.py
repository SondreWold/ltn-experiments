...
class QNLI_Dataset(Dataset):
    ...
    def __init__(self, tokenizer_path_or_name: str = 'bert-base-uncased', root_data_path: str = "./data", split: str = 'train', max_length: int=128):
        '''
            Loads and processes the QNLI dataset
        '''
        assert split in ['dev', 'train', 'test']
        if root_data_path[-1] == '/':
            root_data_path = root_data_path[:-1]
...
class COLA_Dataset(Dataset):
    ...
    def __init__(self, tokenizer_path_or_name: str = 'bert-base-uncased', root_data_path: str = "./data", split: str = 'train', max_length: int=128):
        '''
            Loads and processes the COLA dataset
        '''
        assert split in ['dev', 'train', 'test']
        if root_data_path[-1] == '/':
            root_data_path = root_data_path[:-1]
...
class SST_Dataset(Dataset):
    ...
    def __init__(self, tokenizer_path_or_name: str = 'bert-base-uncased', root_data_path: str = "./data", split: str = 'train', max_length: int=128):
        '''
            Loads and processes the SST-2 dataset
        '''
        assert split in ['dev', 'train', 'test']
        if root_data_path[-1] == '/':
            root_data_path = root_data_path[:-1]
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_path_or_name)
        DATASET_PATH = f'{root_data_path}/SST-2/{split}.tsv'
...
class WNLI_Dataset(Dataset):
    ...
    def __init__(self, tokenizer_path_or_name: str = 'bert-base-uncased', root_data_path: str = "./data", split: str = 'train', max_length: int=128):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_path_or_name)
        DATASET_PATH = f'{root_data_path}/WNLI/{split}.tsv'
...
def dataset_creator(task:str) -> Dataset:
    options = {'RTE': RTE_Dataset, 'MNLI': MNLI_Dataset, 'QNLI': QNLI_Dataset, 'COLA': COLA_Dataset, 'SST': SST_Dataset, 'WNLI': WNLI_Dataset}
